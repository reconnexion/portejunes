...
// activitystreams context alone doesn't type `g1:amount` as an integer or `apods`/`interop`
// -prefixed properties as IRI references.
 ...
 * sending a `Pay` activity. Mirrors `@semapps/activitypub-components`'s `useOutbox`.
 */
const useOutbox = () => {
